...
function START() {
    // 生成第一張圖
    let $board = $('<div>').addClass('chess-board')
    for (let y = 8; y > 0; y--) {
        let $row = $('<div>').addClass('row')
        for (let x = 1; x < 9; x++) {
            let $col = $('<div>').attr('id', String(y) + IndexToCol[x]).addClass('col')
            // 判斷奇偶
            if (y % 2 == 0) {
                if (x % 2 == 0) {
                    $col.addClass('black')
            ...
            // 判斷棋子
            if (InObj(x, y, White)) {
                let piece = getPiece(IndexToCol[x], y)
                piece = (piece == 'king') ? piece : piece.substr(0, piece.length - 1)
    ...
    if (choose_piece == 'king') {
        ...
        if (whiteMove) {
            ...
            if (Black[choose_piece].length == 3) {
                if (Black['rook1'].length == 3 && isEmpty(X - 1, Y) && isEmpty(X - 2, Y)) {
                    pushArray(X, -2, Y, 0, Friend, Enemy)
                    castle.push([X - 2, Y, 1])
                }
                if (Black['rook2'].length == 3 && isEmpty(X + 1, Y) && isEmpty(X + 2, Y) && isEmpty(X + 3, Y)) {
                    console.log(path)
                    pushArray(X, 2, Y, 0, Friend, Enemy)
                    console.log(path)
                    castle.push([X + 2, Y, 2])
    ...
    if (choose_piece.substr(0, choose_piece.length - 1) == 'pawn') {
        ...
        if (whiteMove) {
            ...
            if (White[choose_piece].length == 3) {
                ...
                if (!InObj(X, Y + 2, White) && !InObj(X, Y + 2, Black) && isEmpty(X, Y + 1)) {
